...
const MockWithdrawHandler = artifacts.require('MockWithdrawHandler');
const ReentrancyWithdrawHandler = artifacts.require('ReentrancyWithdrawHandler');
...
contract('KyberStaking', function (accounts) {
  ...
  describe('#Deposit Tests', () => {
    it('Test deposit at beginning of epoch, stakes change as expected', async function () {
      ...
      await Helper.setNextBlockTimestamp(blockToTimestamp(startBlock));
      ...
      Helper.assertEqual(
        mulPrecision(20),
        await stakingContract.getStakesValue(victor, 2),
        'stake at epoch 2 is wrong'
      );

      // delay to start of epoch 4
      await Helper.setNextBlockTimestamp(blockToTimestamp(3 * epochPeriod + startBlock));
      ...
      Helper.assertEqual(
        mulPrecision(20),
        await stakingContract.getStakesValue(victor, 4),
        'stake at epoch 4 is wrong'
      );
      Helper.assertEqual(
        mulPrecision(50),
        await stakingContract.getStakesValue(victor, 5),
        'stake at epoch 5 is wrong'
      );
    });
    ...
    it('Test deposit at end of epoch, stakes change as expected', async function () {
      ...
      Helper.assertEqual(
        mulPrecision(10),
        await stakingContract.getStakesValue(victor, 1),
        'stake at epoch 1 is wrong'
      );
      Helper.assertEqual(0, await stakingContract.getStakesValue(victor, 2), "shouldn't init value at epoch 2");
      Helper.assertEqual(
        mulPrecision(10),
        await stakingContract.getLatestStakeBalance(victor),
        'latest stake balance is wrong'
      );

      await Helper.setNextBlockTimestamp(blockToTimestamp(startBlock) - 1);
      await stakingContract.deposit(mulPrecision(20), {from: victor});
      ...
      await Helper.setNextBlockTimestamp(blockToTimestamp(2 * epochPeriod + startBlock) - 1);
      await stakingContract.deposit(mulPrecision(20), {from: victor});
      ...
      Helper.assertEqual(
        mulPrecision(50),
        await stakingContract.getStakesValue(victor, 3),
        'should have inited value at epoch 3'
      );
      Helper.assertEqual(
        mulPrecision(50),
        await stakingContract.getLatestStakeBalance(victor),
    ...
    it('Test deposit then withdraw at same + different epoch, stakes change as expected', async function () {
      ...
      // delay few epochs
      await Helper.setNextBlockTimestamp(blockToTimestamp(epochPeriod * 6 + startBlock + 1));
      ...
      Helper.assertEqual(
        mulPrecision(60),
        await stakingContract.getStakesValue(victor, 7),
        'stake at epoch 2 is wrong'
      );
      Helper.assertEqual(
        mulPrecision(60),
    ...
    it('Test deposit then withdraw + deposit again at same epoch, stakes change as expected', async function () {
      ...
      // deposit at epoch 0
      await stakingContract.deposit(mulPrecision(50), {from: victor});
      await Helper.setNextBlockTimestamp(blockToTimestamp(startBlock + 2));
      await stakingContract.withdraw(mulPrecision(20), {from: victor});
      Helper.assertEqual(
        mulPrecision(30),
        await stakingContract.getStakesValue(victor, 1),
    ...
    it('Test deposit after full withdraw, stakes change as expected - no delegation', async function () {
      ...
      await Helper.setNextBlockTimestamp(blockToTimestamp(startBlock + 1) + 1);
      await stakingContract.deposit(mulPrecision(50), {from: victor});
    ...
    it('Test deposit after full withdraw, stakes change as expected - with delegation', async function () {
      ...
      Helper.assertEqual(
        mulPrecision(50),
        await stakingContract.getDelegatedStake(mike, 1),
        'delegated stake at epoch 1 is wrong'
      );
      Helper.assertEqual(
        mulPrecision(50),
        await stakingContract.getLatestDelegatedStake(mike),
        'latest delegated stake balance is wrong'
      );
      ...
      Helper.assertEqual(
        mulPrecision(40),
        await stakingContract.getDelegatedStake(mike, 2),
        'delegated stake at epoch 2 is wrong'
      );
      Helper.assertEqual(
        mulPrecision(40),
        await stakingContract.getLatestDelegatedStake(mike),
      ...
      Helper.assertEqual(
        mulPrecision(60),
        await stakingContract.getDelegatedStake(mike, 3),
        'delegated stake at epoch 3 is wrong'
      );
      Helper.assertEqual(
        mulPrecision(60),
        await stakingContract.getLatestDelegatedStake(mike),
    ...
    it('Test deposit few consecutive epochs + after few epochs, stakes change as expected', async function () {
      ...
      for (let id = 0; id < 10; id++) {
        ...
        Helper.assertEqual(
          mulPrecision(totalDeposited - id * 2 - 1),
          await stakingContract.getStakesValue(victor, currentEpoch),
          'stake at cur epoch is wrong, loop: ' + id
        );
        Helper.assertEqual(
          mulPrecision(totalDeposited),
          await stakingContract.getStakesValue(victor, currentEpoch + 1),
          'stake at next epoch is wrong, loop: ' + id
        );
        Helper.assertEqual(
          mulPrecision(totalDeposited),
          await stakingContract.getLatestStakeBalance(victor),
          'latest stake balance is wrong, loop: ' + id
        );
        currentEpoch++;
        await Helper.increaseNextBlockTimestamp(blocksToSeconds(epochPeriod));
      }

      await Helper.mineNewBlockAfter(blocksToSeconds(4 * epochPeriod));
      Helper.assertEqual(
        mulPrecision(totalDeposited),
        await stakingContract.getLatestStakeBalance(victor),
        'latest stake balance is wrong'
      ...
      Helper.assertEqual(
        mulPrecision(totalDeposited),
        await stakingContract.getStake(victor, currentEpoch + 3),
        'stake is wrong'
      );
    });
    ...
    it('Test deposit then delegate, then deposit again, stakes changes as expected', async function () {
      ...
      Helper.assertEqual(
        mulPrecision(100),
        await stakingContract.getDelegatedStakesValue(mike, 1),
        'delegated stake is wrong'
      );
      ...
      Helper.assertEqual(
        mulPrecision(150),
        await stakingContract.getDelegatedStakesValue(mike, 2),
        'delegated stake is wrong'
      );
      Helper.assertEqual(
        mulPrecision(150),
      ...
      // delay few epochs
      await Helper.increaseNextBlockTimestamp(blocksToSeconds(4 * epochPeriod));
      await stakingContract.deposit(mulPrecision(50), {from: victor});
      Helper.assertEqual(
        mulPrecision(150),
        await stakingContract.getDelegatedStake(mike, 5),
        'delegated stake is wrong'
      );
      Helper.assertEqual(
        mulPrecision(200),
        await stakingContract.getDelegatedStakesValue(mike, 6),
        'delegated stake is wrong'
      );
      Helper.assertEqual(
        mulPrecision(200),
    ...
    it('Test deposit then delegate, then delegate again at same + different epoch', async function () {
      ...
      Helper.assertEqual(
        mulPrecision(100),
        await stakingContract.getDelegatedStakesValue(loi, 1),
        'delegated stake is wrong'
      );
      Helper.assertEqual(
        mulPrecision(100),
        await stakingContract.getLatestDelegatedStake(loi),
        'delegated stake is wrong'
      );
      ...
      Helper.assertEqual(
        mulPrecision(100),
        await stakingContract.getDelegatedStakesValue(mike, 5),
        'delegated stake is wrong'
      );
      ...
      Helper.assertEqual(
        mulPrecision(100),
        await stakingContract.getDelegatedStakesValue(loi, 6),
        'delegated stake is wrong'
      );
      Helper.assertEqual(
        mulPrecision(100),
    ...
    it('Test deposit with many stakers', async function () {
      ...
      await Helper.setNextBlockTimestamp(blockToTimestamp(startBlock + epochPeriod));
      ...
      await Helper.mineNewBlockAt(blockToTimestamp(startBlock + 4 * epochPeriod));
      ...
      Helper.assertEqual(
        mulPrecision(300),
        await stakingContract.getLatestStakeBalance(mike),
        'latest stake value is wrong'
      );
      Helper.assertEqual(
        mulPrecision(500),
        await stakingContract.getLatestStakeBalance(victor),
      ...
      Helper.assertEqual(
        mulPrecision(700),
        await stakingContract.getLatestStakeBalance(loi),
        'latest stake value is wrong'
      );
    });
    ...
    it('Test deposit balances change as expected', async function () {
      ...
      Helper.assertEqual(
        expectedStakingBal,
        await kncToken.balanceOf(stakingContract.address),
        'staking balance is not changed as expected'
      );
      ...
      await Helper.increaseNextBlockTimestamp(blocksToSeconds(30));
    ...
    it('Test deposit large amount of tokens, check for overflow', async function () {
      ...
      Helper.assertEqual(
        totalAmount,
        await stakingContract.getLatestDelegatedStake(mike),
        'latest delegated stake is wrong'
      );
  ...
  describe('#Withdrawal Tests', () => {
    it('Test withdraw (partial + full), stakes change as expected - no delegation', async function () {
      ...
      Helper.assertEqual(
        mulPrecision(450),
        await stakingContract.getStakesValue(victor, 1),
        'stake at epoch 1 should be correct'
      );
      Helper.assertEqual(
        mulPrecision(450),
        await stakingContract.getLatestStakeBalance(victor),
        'latest stake balance should be correct'
      );
      ...
      Helper.assertEqual(
        mulPrecision(310),
        await stakingContract.getStakesValue(victor, 5),
        'stake at epoch 5 should be correct'
      );
      Helper.assertEqual(
        mulPrecision(310),
        await stakingContract.getLatestStakeBalance(victor),
      ...
      await Helper.increaseNextBlockTimestamp(blocksToSeconds(5 * epochPeriod));
    ...
    it('Test withdraw (partial + full), stakes change as expected - with delegation', async function () {
      ...
      Helper.assertEqual(
        mulPrecision(450),
        await stakingContract.getDelegatedStakesValue(mike, 1),
        'delegated stake should be correct'
      );
      Helper.assertEqual(
        mulPrecision(450),
        await stakingContract.getLatestDelegatedStake(mike),
        'latest delegated stake balance should be correct'
      );
      ...
      Helper.assertEqual(
        0,
        await stakingContract.getDelegatedStakesValue(mike, 9),
        'delegated stake should be correct'
      );
      Helper.assertEqual(
        0,
    ...
    it('Test withdraw more than current epoch stake, but less than total stake', async function () {
      ...
      Helper.assertEqual(
        mulPrecision(50),
        await stakingContract.getStakesValue(victor, 2),
        'stake at epoch 2 should be correct'
      );
      Helper.assertEqual(
        mulPrecision(50),
        await stakingContract.getLatestStakeBalance(victor),
    ...
    it('Test withdraw before new deposit, stakes change as expected', async function () {
      ...
      Helper.assertEqual(
        mulPrecision(200),
        await stakingContract.getDelegatedStakesValue(mike, 1),
        'delegated stake at epoch 1 should be correct'
      );
      Helper.assertEqual(
        mulPrecision(200),
        await stakingContract.getDelegatedStakesValue(mike, 2),
        'delegated stake at epoch 2 should be correct'
      );
      Helper.assertEqual(
        mulPrecision(200),
        await stakingContract.getLatestDelegatedStake(mike),
        'latest delegated stake should be correct'
      );
    ...
    it('Test withdraw at end and beginning of an epoch', async function () {
      ...
      await Helper.setNextBlockTimestamp(blockToTimestamp(startBlock + epochPeriod) - 1);
    ...
    it('Test withdraw balances change as expected', async function () {
      ...
      await Helper.increaseNextBlockTimestamp(blocksToSeconds(20));
      await stakingContract.withdraw(mulPrecision(100), {from: victor});
      expectedUserBal.iadd(mulPrecision(100));
      expectedStakingBal.isub(mulPrecision(100));
    ...
    it('Test withdraw should call KyberDao handleWithdrawal as expected', async function () {
      ...
      const handler = await MockWithdrawHandler.new();
      await stakingContract.updateWithdrawHandler(handler.address, {from: admin});
      ...
      await stakingContract.deposit(mulPrecision(400), {from: victor});
      await stakingContract.withdraw(mulPrecision(10), {from: victor});
      Helper.assertEqual(0, await handler.values(victor), "shouldn't call dao withdrawal func");

      await Helper.increaseNextBlockTimestamp(blocksToSeconds(epochPeriod));
      await stakingContract.withdraw(mulPrecision(10), {from: victor});
      Helper.assertEqual(mulPrecision(10), await handler.values(victor), 'should call dao withdrawal func');

      await Helper.setNextBlockTimestamp(blockToTimestamp(epochPeriod * 2 + startBlock));

      await stakingContract.deposit(mulPrecision(20), {from: victor});
      await stakingContract.withdraw(mulPrecision(10), {from: victor});
      Helper.assertEqual(mulPrecision(10), await handler.values(victor), "shouldn't call dao withdrawal func");
      await stakingContract.withdraw(mulPrecision(10), {from: victor});
      Helper.assertEqual(mulPrecision(10), await handler.values(victor), "shouldn't call dao withdrawal func");
      await stakingContract.withdraw(mulPrecision(20), {from: victor});
      Helper.assertEqual(mulPrecision(30), await handler.values(victor), 'should call dao withdrawal func');

      await stakingContract.delegate(mike, {from: victor});
      await Helper.setNextBlockTimestamp(blockToTimestamp(3 * epochPeriod + startBlock));

      Helper.assertEqual(mulPrecision(30), await handler.values(victor), 'dao values should be correct');
      Helper.assertEqual(0, await handler.values(mike), 'dao values should be correct');
      ...
      await stakingContract.withdraw(mulPrecision(20), {from: victor});
      Helper.assertEqual(mulPrecision(30), await handler.values(victor), 'dao values should be correct');
      Helper.assertEqual(mulPrecision(15), await handler.values(mike), 'dao values should be correct');

      await stakingContract.delegate(loi, {from: victor});
      await stakingContract.withdraw(mulPrecision(10), {from: victor});
      Helper.assertEqual(mulPrecision(30), await handler.values(victor), 'dao values should be correct');
      Helper.assertEqual(mulPrecision(25), await handler.values(mike), 'dao values should be correct');
      Helper.assertEqual(0, await handler.values(loi), 'dao values should be correct');
      ...
      Helper.assertEqual(mulPrecision(30), await handler.values(victor), 'dao values should be correct');
      Helper.assertEqual(mulPrecision(25), await handler.values(mike), 'dao values should be correct');
      Helper.assertEqual(mulPrecision(10), await handler.values(loi), 'dao values should be correct');

      await stakingContract.delegate(victor, {from: victor});
      // move to next epoch
      await Helper.increaseNextBlockTimestamp(blocksToSeconds(epochPeriod));
      await stakingContract.withdraw(mulPrecision(10), {from: victor});
      Helper.assertEqual(mulPrecision(40), await handler.values(victor), 'dao values should be correct');
      Helper.assertEqual(mulPrecision(25), await handler.values(mike), 'dao values should be correct');
      Helper.assertEqual(mulPrecision(10), await handler.values(loi), 'dao values should be correct');
    });
    ...
    it('Test handleWithdrawal should revert sender is not staking', async () => {
      ...
      await expectRevert(stakingContract.handleWithdrawal(victor, mulPrecision(100), 0), 'only staking contract');
      await stakingContract.withdraw(mulPrecision(100), {from: victor});
    });

    // TODO: convert this test with a full setup and to another file
    it('Test withdraw gas usages', async function () {
      ...
      await stakingContract.delegate(mike, {from: victor});
      await Helper.setNextBlockTimestamp(blockToTimestamp(6 * epochPeriod + startBlock));
      tx = await stakingContract.withdraw(mulPrecision(10), {from: victor});
      logInfo('Withdraw has delegation: init 2 epoch data + has penalty amount, gas used: ' + tx.receipt.gasUsed);

      await Helper.setNextBlockTimestamp(blockToTimestamp(7 * epochPeriod + startBlock));
      tx = await stakingContract.withdraw(mulPrecision(10), {from: victor});
  ...
  describe('#Delegate Tests', () => {
    it('Test delegate, representative and stake change as expected', async function () {
      ...
      Helper.assertEqual(
        mike,
        await stakingContract.getLatestRepresentative(victor),
        'latest representative is incorrect'
      );
      Helper.assertEqual(
        mulPrecision(50),
        await stakingContract.getLatestDelegatedStake(mike),
        'latest delegated stake is incorrect'
      );
      Helper.assertEqual(0, await stakingContract.getDelegatedStakesValue(mike, 5), 'delegated stake is incorrect');
      Helper.assertEqual(
        mulPrecision(50),
        await stakingContract.getDelegatedStakesValue(mike, 6),
        'delegated stake is incorrect'
      );
      ...
      await Helper.setNextBlockTimestamp(blockToTimestamp(startBlock + 8 * epochPeriod) - 1);
    ...
    it('Test delegate same address many times', async function () {
      ...
      Helper.assertEqual(
        mulPrecision(50),
        await stakingContract.getDelegatedStake(mike, 10),
        'delegated stake is incorrect'
      );
      Helper.assertEqual(
        mulPrecision(50),
    ...
    it('Test delegate, then delegate back to yourself', async function () {
      ...
      await Helper.mineNewBlockAt(blockToTimestamp(4 * epochPeriod + startBlock));
      ...
      Helper.assertEqual(
        mulPrecision(50),
        await stakingContract.getDelegatedStake(mike, 4),
        'delegated stake is incorrect'
      );
      Helper.assertEqual(
        mulPrecision(50),
      ...
      Helper.assertEqual(
        0,
        await stakingContract.getLatestDelegatedStake(victor),
        'latest delegated stake is incorrect'
      );
    ...
    it('Test delegate then deposit more at current + next + after few epochs', async function () {
      ...
      Helper.assertEqual(
        mulPrecision(200),
        await stakingContract.getDelegatedStakesValue(mike, 7),
        'delegated stake is incorrect'
      );

      await Helper.increaseNextBlockTimestamp(blocksToSeconds(epochPeriod * 4));
      await stakingContract.deposit(mulPrecision(200), {from: victor});

      Helper.assertEqual(
        mulPrecision(200),
        await stakingContract.getDelegatedStakesValue(mike, 10),
        'delegated stake is incorrect'
      );
      Helper.assertEqual(
        mulPrecision(400),
        await stakingContract.getDelegatedStakesValue(mike, 11),
        'delegated stake is incorrect'
      );
    });
    ...
    it('Test delegate from many addresses, stakes change as expected', async function () {
      ...
      Helper.assertEqual(
        mulPrecision(190),
        await stakingContract.getDelegatedStakesValue(loi, 4),
        'delegated stake is wrong'
      );
      Helper.assertEqual(
        mulPrecision(190),
      ...
      Helper.assertEqual(
        mulPrecision(100),
        await stakingContract.getDelegatedStakesValue(loi, 5),
        'delegated stake is wrong'
      );
      Helper.assertEqual(
        mulPrecision(100),
    ...
    it('Test delegate then withdraw, stakes change as expected', async function () {
      ...
      Helper.assertEqual(
        mulPrecision(300),
        await stakingContract.getDelegatedStakesValue(loi, 1),
        'delegated stake is not correct'
      );
      Helper.assertEqual(
        mulPrecision(300),
        await stakingContract.getLatestDelegatedStake(loi),
      ...
      Helper.assertEqual(
        mulPrecision(200),
        await stakingContract.getDelegatedStakesValue(loi, 2),
        'delegated stake is not correct'
      );
      Helper.assertEqual(
        mulPrecision(200),
      ...
      await Helper.mineNewBlockAfter(blocksToSeconds(4 * epochPeriod) - 2);

      Helper.assertEqual(
        mulPrecision(200),
        await stakingContract.getDelegatedStake(loi, 4),
        'delegated stake is not correct'
      );
      Helper.assertEqual(
        mulPrecision(200),
        await stakingContract.getDelegatedStake(loi, 5),
        'delegated stake is not correct'
      );
    ...
    it('Test delegate at end and beginning of an epoch', async function () {
      ...
      await Helper.setNextBlockTimestamp(blockToTimestamp(epochPeriod + startBlock));
      ...
      Helper.assertEqual(
        mulPrecision(300),
        await stakingContract.getDelegatedStakesValue(loi, 3),
        'delegated stake is wrong'
      );
    ...
    it('Test delegate circulation, data changes as expect', async function () {
      ...
      Helper.assertEqual(
        mulPrecision(300),
        await stakingContract.getDelegatedStakesValue(victor, 1),
        'delegated stake is wrong'
      );
      Helper.assertEqual(mike, await stakingContract.getRepresentativeValue(victor, 1), 'representative is wrong');
      Helper.assertEqual(
        mike,
        await stakingContract.getLatestRepresentative(victor),
        'latest representative is wrong'
      );
      ...
      Helper.assertEqual(
        mulPrecision(250),
        await stakingContract.getDelegatedStakesValue(victor, 2),
        'delegated stake is wrong'
      );
      Helper.assertEqual(
        mulPrecision(100),
      ...
      await Helper.mineNewBlockAfter(blocksToSeconds(epochPeriod * 4));
      Helper.assertEqual(
        mulPrecision(250),
        await stakingContract.getDelegatedStake(victor, 4),
        'delegated stake is wrong'
      );
      Helper.assertEqual(
        mulPrecision(100),
      ...
      Helper.assertEqual(
        mulPrecision(250),
        await stakingContract.getDelegatedStakesValue(victor, 5),
        'delegated stake is wrong'
      );
      Helper.assertEqual(
        mulPrecision(100),
    ...
    it('Test delegate gas usages', async function () {
      ...
      await stakingContract.delegate(victor, {from: victor});
      await Helper.setNextBlockTimestamp(blockToTimestamp(5 * epochPeriod + startBlock));
      tx = await stakingContract.delegate(mike, {from: victor});
      ...
      await Helper.setNextBlockTimestamp(blockToTimestamp(8 * epochPeriod + startBlock));
      tx = await stakingContract.delegate(mike, {from: victor});
      logInfo('Delegate has stake: init 2 epochs data + from mike to loi, gas used: ' + tx.receipt.gasUsed);
      await stakingContract.delegate(victor, {from: victor});
      await Helper.setNextBlockTimestamp(blockToTimestamp(9 * epochPeriod + startBlock));
      tx = await stakingContract.delegate(mike, {from: victor});
      logInfo('Delegate has stake: init 1 epoch data + from mike to loi, gas used: ' + tx.receipt.gasUsed);
      await Helper.setNextBlockTimestamp(blockToTimestamp(10 * epochPeriod + startBlock));
      // to make init 2 epochs data
      await stakingContract.delegate(victor, {from: victor});
      tx = await stakingContract.delegate(loi, {from: victor});
  ...
  describe('#GetFunctions Tests', () => {
    it('Test get functions return default value when calling epoch > current epoch + 1', async function () {
      ...
      Helper.assertEqual(
        zeroAddress,
        await stakingContract.getRepresentative(victor, 100),
        'get stakes should return 0'
      );
    });
    ...
    it('Test getStake returns correct data', async function () {
      ...
      Helper.assertEqual(
        mulPrecision(50),
        await stakingContract.getStake(victor, 1),
        'get stakes should return correct data'
      );

      await stakingContract.deposit(mulPrecision(20), {from: victor});

      await Helper.mineNewBlockAt(blockToTimestamp(3 * epochPeriod + startBlock));

      // get data current + next epoch
      Helper.assertEqual(
        mulPrecision(70),
        await stakingContract.getStake(victor, 4),
        'get stakes should return correct data'
      );
      Helper.assertEqual(
        mulPrecision(70),
        await stakingContract.getStake(victor, 5),
        'get stakes should return correct data'
      );

      currentBlock = await Helper.getCurrentBlock();
      await Helper.mineNewBlockAfter(blocksToSeconds(6 * epochPeriod));

      // get data for pass epoch
      Helper.assertEqual(
        mulPrecision(70),
        await stakingContract.getStake(victor, 7),
        'get stakes should return correct data'
      );
      Helper.assertEqual(
        mulPrecision(70),
        await stakingContract.getStake(victor, 8),
        'get stakes should return correct data'
      );
      ...
      Helper.assertEqual(
        mulPrecision(70),
        await stakingContract.getStake(victor, 10),
        'get stakes should return correct data'
      );
      Helper.assertEqual(
        mulPrecision(100),
        await stakingContract.getStake(victor, 11),
        'get stakes should return correct data'
      );
    ...
    it('Test getDelegatedStake returns correct data', async function () {
      ...
      Helper.assertEqual(
        0,
        await stakingContract.getDelegatedStake(victor, 0),
        'get delegated stakes should return 0'
      );
      Helper.assertEqual(
        0,
        await stakingContract.getDelegatedStake(victor, 1),
        'get delegated stakes should return correct data'
      );
      ...
      await Helper.mineNewBlockAfter(blocksToSeconds(6 * epochPeriod) - 2);

      // get data for past epoch
      Helper.assertEqual(
        mulPrecision(70),
        await stakingContract.getDelegatedStake(mike, 8),
        'get delegated stakes should return correct data'
      );
      ...
      Helper.assertEqual(
        mulPrecision(70),
        await stakingContract.getDelegatedStake(mike, 9),
        'get delegated stakes should return correct data'
      );
      Helper.assertEqual(
        mulPrecision(100),
      ...
      Helper.assertEqual(
        0,
        await stakingContract.getDelegatedStake(loi, 8),
        'get delegated stakes should return correct data'
      );
      Helper.assertEqual(
        mulPrecision(70),
      ...
      Helper.assertEqual(
        mulPrecision(100),
        await stakingContract.getDelegatedStake(mike, 10),
        'get delegatedstakes should return correct data'
      );
      Helper.assertEqual(
        0,
        await stakingContract.getDelegatedStake(mike, 11),
        'get delegatedstakes should return correct data'
      );
      Helper.assertEqual(
        0,
        await stakingContract.getDelegatedStake(loi, 10),
        'get delegated stakes should return correct data'
      );
      Helper.assertEqual(
        mulPrecision(100),
        await stakingContract.getDelegatedStake(loi, 11),
        'get delegated stakes should return correct data'
      );

      Helper.assertEqual(
        0,
        await stakingContract.getDelegatedStake(victor, 100),
        'get delegated stake should return correct data'
      );
    });
    ...
    it('Test getRepresentative returns correct data', async function () {
      ...
      Helper.assertEqual(
        mike,
        await stakingContract.getRepresentative(mike, 0),
        'get representative should return correct data'
      );
      Helper.assertEqual(
        mike,
        await stakingContract.getRepresentative(mike, 1),
        'get representative should return correct data'
      );
      Helper.assertEqual(
        victor,
        await stakingContract.getRepresentative(victor, 0),
        'get representative should return correct data'
      );
      Helper.assertEqual(
        victor,
        await stakingContract.getRepresentative(victor, 1),
        'get representative should return correct data'
      );
      ...
      Helper.assertEqual(
        mike,
        await stakingContract.getRepresentative(victor, 4),
        'get representative should return correct data'
      );
      Helper.assertEqual(
        mike,
        await stakingContract.getRepresentative(victor, 5),
        'get representative should return correct data'
      );
      ...
      Helper.assertEqual(
        mike,
        await stakingContract.getRepresentative(mike, 7),
        'get representative should return correct data'
      );
      ...
      Helper.assertEqual(
        mike,
        await stakingContract.getRepresentative(victor, 7),
        'get representative should return correct data'
      );
      ...
      Helper.assertEqual(
        mike,
        await stakingContract.getRepresentative(victor, 10),
        'get representative should return correct data'
      );
      Helper.assertEqual(
        loi,
        await stakingContract.getRepresentative(victor, 11),
        'get representative should return correct data'
      );
    ...
    it('Test get latest stake returns correct data', async function () {
      ...
      await Helper.increaseNextBlockTimestamp(blocksToSeconds(35));
      await stakingContract.withdraw(mulPrecision(10), {from: victor});
      Helper.assertEqual(0, await stakingContract.getLatestStakeBalance(victor), 'latest stake is wrong');
      await stakingContract.deposit(mulPrecision(20), {from: victor});
      Helper.assertEqual(
    ...
    it('Test get latest delegated stake returns correct data', async function () {
      ...
      await Helper.increaseNextBlockTimestamp(blocksToSeconds(25));
      await stakingContract.delegate(loi, {from: victor});
      Helper.assertEqual(0, await stakingContract.getLatestDelegatedStake(mike), 'latest delegated stake is wrong');
      Helper.assertEqual(
        mulPrecision(50),
    ...
    it('Test get representative returns correct data', async function () {
      ...
      Helper.assertEqual(
        victor,
        await stakingContract.getLatestRepresentative(mike),
        'latest representative is wrong'
      );
      Helper.assertEqual(
        mike,
    ...
    it('test get staker data returns correct data', async function () {
      ...
      // delay to epoch 1
      await Helper.mineNewBlockAfter(blocksToSeconds(25));
    ...
    it('Test get staker data for current epoch called by KyberDao', async function () {
      ...
      Helper.assertEqual(
        false,
        await stakingContract.getHasInitedValue(victor, 2),
        "shouldn't inited value for epoch 2"
      );

      // victor: stake (100), delegated stake (0), representative (victor)
      await checkInitAndReturnStakerDataForCurrentEpoch(victor, mulPrecision(100), 0, victor, kyberDao);
      ...
      await checkInitAndReturnStakerDataForCurrentEpoch(mike, 0, 0, mike, kyberDao);

      await stakingContract.deposit(mulPrecision(200), {from: mike});

      await Helper.setNextBlockTimestamp(blockToTimestamp(2 * epochPeriod + startBlock));

      // victor: stake (100), delegated stake (0), representative (mike)
      await checkInitAndReturnStakerDataForCurrentEpoch(victor, mulPrecision(100), 0, mike, kyberDao);

      // mike: stake (200), delegated stake (100), representative (mike)
      await checkInitAndReturnStakerDataForCurrentEpoch(mike, mulPrecision(200), mulPrecision(100), mike, kyberDao);
      ...
      await checkInitAndReturnStakerDataForCurrentEpoch(loi, 0, 0, loi, kyberDao);
      ...
      await checkInitAndReturnStakerDataForCurrentEpoch(mike, mulPrecision(200), 0, mike, kyberDao);

      // loi: stake (0), delegated stake (90), representative (loi)
      await checkInitAndReturnStakerDataForCurrentEpoch(loi, 0, mulPrecision(110), loi, kyberDao);
    });
  });
  ...
  describe('#Revert Tests', () => {
    it('Test constructor should revert with invalid arguments', async function () {
      // knc is 0
      await expectRevert(
        StakingContract.new(admin, zeroAddress, blocksToSeconds(20), blockToTimestamp(currentBlock + 10)),
        'ctor: kncToken 0'
      );
      // epoch period is 0
      await expectRevert(
        StakingContract.new(admin, kncToken.address, blocksToSeconds(0), blockToTimestamp(currentBlock + 10)),
        'ctor: epoch period is 0'
      );
      // start timestamp is in the past
      await expectRevert(
        StakingContract.new(admin, kncToken.address, blocksToSeconds(20), blockToTimestamp(currentBlock - 1)),
    ...
    it('Test deposit should revert when amount is 0', async function () {
      ...
      await expectRevert(stakingContract.deposit(0, {from: victor}), 'deposit: amount is 0');
    ...
    it('Test withdraw should revert amount is 0', async function () {
      ...
      await expectRevert(stakingContract.withdraw(0, {from: victor}), 'withdraw: amount is 0');
      await stakingContract.withdraw(mulPrecision(10), {from: victor});
    });
    ...
    it('Test withdraw should revert when amount more than current deposited amount', async function () {
      ...
      Helper.assertEqual(0, await stakingContract.getLatestStakeBalance(victor), 'latest stake is incorrect');
      Helper.assertEqual(
        mulPrecision(800),
        await stakingContract.getLatestStakeBalance(loi),
        'latest stake is incorrect'
    ...
    it('Test delegate should revert when delegate to address 0', async function () {
      ...
      await expectRevert(stakingContract.delegate(zeroAddress, {from: victor}), 'delegate: representative 0');
      await stakingContract.delegate(mike, {from: victor});
    });
  });
  ...
  describe("Test Withdrawal shouldn't revert", () => {
    ...
    it("Test withdraw shouldn't revert when WithdrawHandler does not have handleWithdrawl func", async function () {
      await deployStakingContract(10, currentBlock + 10);
      await stakingContract.updateWithdrawHandler(accounts[1], {from: admin});
    ...
    it("Test withdraw shouldn't revert with re-entrancy from KyberDao", async () => {
      ...
      let attacker = await ReentrancyWithdrawHandler.new(stakingContract.address, kncToken.address);
      await kncToken.transfer(attacker.address, mulPrecision(100));
      await stakingContract.updateWithdrawHandler(attacker.address, {from: admin});
      await attacker.deposit(mulPrecision(80));

      // delay to epoch 1, so withdraw will call KyberDao to handle withdrawal
      await Helper.setNextBlockTimestamp(blockToTimestamp(startBlock));

      // partial withdraw, dao will try to re-enter withdraw
      await attacker.withdraw(mulPrecision(10));
      // all data should be updated
      await Helper.assertEqual(mulPrecision(70), await stakingContract.getLatestStakeBalance(attacker.address));
      await Helper.assertEqual(mulPrecision(70), await stakingContract.getStake(attacker.address, 1));
      await Helper.assertEqual(mulPrecision(70), await stakingContract.getStake(attacker.address, 2));

      // full withdraw, no reentrant
      await attacker.withdraw(mulPrecision(70));
      // stake and latest stake should be updated
      await Helper.assertEqual(0, await stakingContract.getLatestStakeBalance(attacker.address));
      await Helper.assertEqual(0, await stakingContract.getStake(attacker.address, 1));
      await Helper.assertEqual(0, await stakingContract.getStake(attacker.address, 2));
    });
  ...
  describe('#Malicious Staking', () => {
    ...
    it('Test delegate should revert, delegated stake is small', async function () {
      ...
      await expectRevert.unspecified(stakingContract.delegate(loi, {from: victor}));
